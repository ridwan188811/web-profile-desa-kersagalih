<x-admin-layout>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Edit Potensi Desa</h1>
    ...
    <form action="{{ route('admin.potensis.update', $potensi->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden" x-data="{ categoryMode: 'select' }">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $potensi->title) }}" required
                        ...
                        <div x-show="categoryMode === 'select'">
                            <select name="category" id="category_select" :required="categoryMode === 'select'"
                                ...
                                @php
                                    ...
                                    $allCats = collect($defaultCategories)->push($potensi->category)->unique()->sort();
                                @endphp
                                @foreach($allCats as $cat)
                                    <option value="{{ $cat }}" {{ old('category', $potensi->category) == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="pt-2 flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $potensi->is_active) ? 'checked' : '' }} class="w-5 h-5 text-[#1e3a8a] rounded border-slate-300 focus:ring-[#1e3a8a]">
                        <label for="is_active" class="text-sm font-bold text-slate-700">Tampilkan di Website Publik</label>
                    </div>
                </div>

                <!-- Kolom Kanan: Gambar -->
                <div x-data="{ imagePreview: '{{ $potensi->image ? asset('storage/' . $potensi->image) : '' }}' }">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Foto Utama Potensi (Abaikan jika tidak diubah)</label>
