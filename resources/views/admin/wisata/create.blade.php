<x-admin-layout>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Tambah wisata Baru</h1>
    ...
    <form action="{{ route('admin.wisata.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden" x-data="{ locationMode: 'select' }">
        @csrf
        ...
        <div class="p-6 sm:p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div x-show="locationMode === 'select'">
                            <select name="location" id="location_select" :required="locationMode === 'select'"
                                ...
                                @foreach(['Pertanian', 'Peternakan', 'Pariwisata', 'UMKM', 'Kerajinan', 'Kesenian'] as $cat)
                                    <option value="{{ $cat }}" {{ old('location') == $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div x-data="{ imagePreview: null }">
                    <label class="block text-sm font-bold text-slate-700 mb-2">Foto Utama wisata <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <p class="text-xs text-slate-500 mb-3">Tuliskan keunggulan, lokasi, atau narasi menarik tentang wisata ini.</p>
