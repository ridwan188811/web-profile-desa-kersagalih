<x-admin-layout>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Tambah Anggota SOTK</h1>
            <p class="text-slate-500 mt-1">Masukkan data aparatur desa atau anggota lembaga baru.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.personils.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden" x-data="{ categoryMode: 'select' }">
        @csrf
        ...
        <div class="p-6 sm:p-8 flex flex-col lg:flex-row gap-8">
            ...
            <div class="w-full lg:w-1/3 shrink-0" x-data="{ imagePreview: null }">
                <label class="block text-sm font-bold text-slate-700 mb-2">Foto Formal (Wajib) <span class="text-red-500">*</span></label>
                <div class="p-4 border-2 border-slate-200 border-dashed rounded-2xl bg-slate-50 text-center relative hover:bg-slate-100 transition-colors cursor-pointer">
                    <!-- Area Foto Berlatar Merah Sesuai Request -->
                    ...
                    <input id="image" name="image" type="file" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept="image/png, image/jpeg, image/jpg" 
            ...
            <div class="w-full lg:w-2/3 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                        ...
                        placeholder="Contoh: Bpk. ASEP, S.E.">
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="position" id="position" value="{{ old('position') }}" required
                            class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[#1e3a8a] transition-all" 
                            placeholder="Contoh: Kepala Desa / Kaur Umum">
                    </div>
                </div>
                ...
                <!-- Status Aktif (Hidden/True by default for now, or we can use a checkbox) -->
