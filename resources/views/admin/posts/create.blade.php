<x-admin-layout>
    ...
    <div class="flex items-center gap-3 mb-8">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Tulis Berita Baru</h1>
            <p class="text-slate-500 mt-1">Publikasikan informasi terbaru untuk masyarakat.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden" x-data="{ categoryMode: 'select' }">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        class="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:ring-2 focus:ring-[#1e3a8a] focus:border-[#1e3a8a] focus:bg-white transition-all appearance-none">
                        <option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Tampilkan Sekarang (Published)</option>
                ...
                <div class="flex items-center justify-center w-full">
                    <label for="image" class="flex flex-col items-center justify-center w-full h-64 border-2 border-slate-300 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors relative overflow-hidden">
                        ...
                        <div x-show="!imagePreview" class="flex flex-col items-center justify-center pt-5 pb-6 text-slate-500">
                            ...
                            <p class="mb-2 text-sm font-bold text-slate-700">Klik untuk mengunggah gambar</p>
